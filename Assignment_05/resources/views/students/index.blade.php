...
<body>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form name="sendEmailForm">
                    <div class="modal-body">
                        <div class="form-group mb-3">
                            <label for="name" class="col-12 control-label mb-2">Email</label>
                            <div class="col-12">
                                <input type="text" name="sendEmail" id="sendGetEmail" class="form-control" />
                            </div>
                        </div>
                        <div class="modal-footer">
    ...
    <script>
                ...
                .then(response => {
                    ...
                    if (response.data.msg == "Data Created Successfully") {
                        ...
                        tableBody.innerHTML += '<tr>' +
                            ...
                            '<button onclick="generatePdf(' + student.id +
                            ')" type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#sendMail">Mail</button>' +
                            '</td>' +
                            '</tr>';
        ...
        function generatePdf(id) {
            studentId = id;
            axios.get('/api/students/pdf/' + studentId).
            then(response => {
                if (response.data.msg == "Students Datas Send To Pdf") {
                    document.getElementById("sendGetEmail").value = response.data.pdfStudent.email;
                    document.getElementById("sendGetEmail").setAttribute("disabled", true);
                }
            }).
        ...
        let sendEmailInput = sendEmailForm['sendEmail'];

        sendEmailForm.onsubmit = function(e) {
            e.preventDefault();
            axios.post('/api/students/email/' + studentId).
            then(response => {
                console.log(response);
