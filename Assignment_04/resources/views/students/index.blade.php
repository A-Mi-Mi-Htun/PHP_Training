...
<body>
            ...
            <div class="col-sm-8 mt-3" id="major">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <button type="button" class="btn btn-primary d-inline-block" data-bs-toggle="modal" data-bs-target="#createStudent">Create</button>    
                    </div>
                    <div class="card-body">
    ...
    <script>
        ...
        //POST
        let createStudent = document.forms['createForm'];
        ...
        createStudent.onsubmit = function(e) {
            ...
            .then(response => {
                console.log(response);
                if (response.data.createStudent.msg == "Data Created successfully") {
                    alertMsg(response.data.createStudent.msg);
                    createStudent.reset();
                    displayStudent(response.data.createStudent);
                } else {
                        document.getElementById('msgName').innerHTML = (createName.value == '') ? '<i class="text-danger">'+response.data.msg.name+'</i>': '';

                        document.getElementById('msgEmail').innerHTML = (createEmail.value == '') ? '<i class="text-danger">'+response.data.msg.email+'</i>': '';

                        document.getElementById('msgPhone').innerHTML = (createPhone.value == '') ? '<i class="text-danger">'+response.data.msg.phone+'</i>': '';

                        document.getElementById('msgAddress').innerHTML = (createAddress.value == '') ? '<i class="text-danger">'+response.data.msg.address+'</i>': '';

                        document.getElementById('msgMajor').innerHTML = (createMajor.value == '') ? '<i class="text-danger">'+response.data.msg.major+'</i>': '';
                }
            })
            .catch(error => {
        ...
        //EDIT AND UPDATE
        let editStudent = document.forms['editForm'];
            ...
            .then(response => {
                ...
                for (let i = 0; i < nameList.length; i++) {
                    if (nameList[i].innerHTML == oldName) {
                        ...
                        majorList[i].innerHTML = editMajor.value;
                ...
                .then(response => {
                    ...
                    for (let i = 0; i < nameList.length; i++) {
                        if (nameList[i].innerHTML == response.data.deleteStudent.name) {
                            idList[i].style.display = nameList[i].style.display = phoneList[i].style.display = emailList[i].style.display = addressList[i].style.display = majorList[i].style.display = editList[i].style.display = deleteList[i].style.display = "none";
                        }
                    }
                })
                .catch(error => console.log(error));
                ...
                .then(response => {
                    ...
                    if (response.data.msg == "Data Created successfully") {
                        ...
                        displayMajor(response.data.createMajor);
                            ...
                            document.getElementById('msgName').innerHTML = (majorName.value == '') ? '<i class="text-danger">'+response.data.msg.name+'</i>': '';
                    }
                })
                .catch(error => {
                    console.log(error.response);
        ...
        function displayStudent(data) {
            tableBody.innerHTML += '<tr>'+
                                        ...
                                        '</td>'+
                                        '<td class="editList">'+
                                            '<button onclick="editBtn('+ data.id +')" type="button" class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#editStudent">Edit</button>'+
                                        '</td>'+
                                        '<td class="deleteList">'+
                                            '<button onclick="deleteBtn('+ data.id +')" class="btn btn-sm btn-danger">Delete</button>'+
                                        '</td>'+
                                    '</tr>';
        }
        ...
        function displayMajor(data) {
            majorBody.innerHTML += '<tr>'+
                                        ...
                                        '</td>'+
                                        '<td class="btnList">'+
                                            '<button onclick="editBtn('+ data.id +')" type="button" class="btn btn-sm btn-success me-3" data-bs-toggle="modal" data-bs-target="#editMajor">Edit</button>'+
                                            '<button onclick="deleteBtn('+ data.id +')" class="btn btn-sm btn-danger">Delete</button>'+
                                        '</td>'+
                                    '</tr>';
        }
        ...
        function alertMsg(msg) {
            document.getElementById('successMsg').innerHTML = '<div class="alert alert-success alert-dismissible fade show" role="alert"><strong>'+msg+'</strong>'+
